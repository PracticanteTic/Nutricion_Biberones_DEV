...
if (!isset($_SESSION["nombre"])) {
  ...
  $pageTitle = "Consolidado de solicitud de biberones"; // Titulo pagina html
  require 'template/header.php'; // Trae la vista con el encabezado

  if ($_SESSION['FormulasPreparadas'] == 1) {


    //SE USA PARA QUE EL ROL DESIGNADO SEA QUIEN PUEDA MODIFICARLO Y NADIE MÁS
...
?>
    ...
    <!-- DataTales Example -->
    <!--<div class="shadow p-3 mt-5 bg-body rounded"> Inicio original del contenedor con el margin top de boostrap mt-5-->
    <div class="shadow p-3 bg-body rounded" style="margin-top:80px;">
      <div class="col-md-12">
        <div class="row">
          <h1 class="text-success">Información de biberones</h1>
        </div>
        <br>
            ...
            <tbody id="table_body">
              ...
              while ($row = mysqli_fetch_assoc($result_biberones)) { ?>
                ...
                <tr class="no-print"> <!--name="print[]"-->
                  ...
                  <td>
                    <input type="number" <?php if ($onlyRead == true) { ?> readonly <?php }
                    ?>
      ...
      if(value><?php echo $row['numero_biberones'] ?>) 
        value = <?php echo $row['numero_biberones'] ?>" value=0 style="width: 40px">
                  </td> <!-- Tambi�n se modific� ese mismo d�a -->


                  <td> <input type="number" id="Tanda_pm<?php echo $row["Id_biberon"] ?>" name="tanda_Etiquetas[]"" style=" width: 40px" value=0 min="0" max="<?php echo $row['numero_biberones'] ?>" <?php if ($row['id_estado'] == 4 || $row['id_estado'] == 9) { ?> disabled <?php } ?>></td> <!-- Nueva modificaci�n el d�a 25 de Junio del 2024 -->
...
<script src="./scripts/DeshabilitarRetroceso.js">
  //Evita Retroceder <!-- evita que al enviar formulario al dar regresar en el navegador, no se cargue la información enviada -->
</script>
